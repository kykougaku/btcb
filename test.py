...
for idx,(macd,signal,before_macd,before_signal,close,upsigma4,losigma4,smass,emass,before_smass,before_emass) in enumerate(zip(hist["MACD"].values,hist["Signal"].values,hist["before_MACD"].values,hist["before_Signal"].values,hist["Close"].values,hist["upper_sigma4"].values,hist["lower_sigma4"].values,hist["SMA"+str(sma)].values,hist["EMA"+str(ema)].values,hist["before_SMA"+str(sma)].values,hist["before_EMA"+str(ema)].values)):
    ...
    #if(before_macd<before_signal and macd>signal and (-70000.0>(emass-close) or (emass-close)>100000) and mode==2): 
...
grad_datas = pd.DataFrame(list(zip(macd_grads,signal_grads,profits,smas,emas,closes,sma_grads,ema_grads)),columns=["MACD_grad","Signal_grad","profit","SMA"+str(sma),"EMA"+str(ema),"Close","SMA_grad","EMA_grad"])

fig3 = plt.figure()
#plt.scatter(grad_datas["MACD_grad"]*((grad_datas["MACD_grad"]-grad_datas["Signal_grad"])**2.0),grad_datas["profit"])
plt.scatter(grad_datas["EMA"+str(ema)]-grad_datas["Close"],grad_datas["profit"])
plt.show()
fig4 = plt.figure()
plt.scatter(grad_datas["EMA_grad"],grad_datas["profit"])
plt.show()
fig5 = plt.figure()
plt.scatter(grad_datas["EMA"+str(ema)]-grad_datas["Close"],grad_datas["SMA_grad"],c=grad_datas["profit"],cmap="jet")
plt.colorbar()
plt.show()